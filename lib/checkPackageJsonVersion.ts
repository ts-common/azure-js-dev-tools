...
function checkPackageJsonVersion(startPath: string): number {
  ...
  if (!packageJsonFilePath) {
    console.error(`##[error]Could not find a package.json file at "${startPath}" or in any of its parent folders.`);
    exitCode = 1;
  } else {
    ...
    if (isPackageJsonPublished(packageJson)) {
      console.error(`##[error]A package with the name "${packageJson.name}" and the version "${packageJson.version}" already exists in NPM.`);
      exitCode = 2;
    } else {
